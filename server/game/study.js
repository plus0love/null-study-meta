 ...
 *
 * 이벤트: 'saved' { nickname, playerId, seconds, session }, 'discarded' { nickname, seconds },
 *         'attendance' { nickname, playerId, streak, weekDays, attendedToday, inserted },
...
class StudyTracker extends EventEmitter {
  constructor({ store, tz = DEFAULT_TZ, now = () => Date.now(), log = console, goalOf = () => null, minSeconds = MIN_SESSION_SECONDS, tickMs = TICK_MS, autoTick = true } = {}) {
    ...
    this.live = new Map(); // nickname → { nickname, playerId, startedAt, attended }
  ...
  start(player) {
    const s = { nickname: player.nickname, playerId: player.id, startedAt: this.now(), attended: false };
  ...
  /** 세션 종료 → 60초 이상이면 저장. 반환: 저장된 세션 또는 null */
    ...
    const p = (async () => {
      ...
      this.statsCache = null;
      this.emit('saved', { nickname, playerId: s.playerId, seconds, session, reason });
      if (!s.attended) await this.attend(nickname, s.playerId);
      this.checkGoal(nickname, s.playerId);
      return session;
    })().catch((err) => {
  ...
  async attend(nickname, playerId) {
  ...
  checkGoal(nickname, playerId) {
    ...
    this.goalReached.add(key);
    this.emit('goalReached', { nickname, playerId, todaySeconds, targetMinutes: goal.targetMinutes });
    return true;
  }
  ...
  async tick() {
    ...
    for (const s of [...this.live.values()]) {
      if (!s.attended && this.liveSeconds(s.nickname) >= this.minSeconds) await this.attend(s.nickname, s.playerId);
      this.checkGoal(s.nickname, s.playerId);
    }
  }

  /**
   * 랭킹용 통계. players: 접속 중 플레이어 목록(nickname, connected).
   * 반환: [{ nickname, todaySeconds, weekSeconds, streak, weekDays, live, online }] (진행 중 세션 포함, 3초 캐시)
   */
  async stats(players = []) {
    const now = this.now();
    if (this.statsCache && now - this.statsCache.at < STATS_CACHE_MS) return this.statsCache.rows;
    const [totals, att] = await Promise.all([this.store.studyTotals({ tz: this.tz, now }), this.store.attendanceStats({ tz: this.tz, now })]);
    const rows = new Map();
    const row = (n) => {
      if (!rows.has(n)) rows.set(n, { nickname: n, todaySeconds: 0, weekSeconds: 0, streak: 0, weekDays: 0, live: false, online: false });
      return rows.get(n);
    };
    ...
    for (const p of players) if (p.connected) row(p.nickname).online = true;
